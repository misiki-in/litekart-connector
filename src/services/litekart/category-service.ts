...
export class CategoryService extends BaseService {
	...
	async fetchFooterCategories({ page = 1, q = '', sort = '-createdAt' }) {
		return this.get(`/api/categories?page=${page}&q=${q}&sort=${sort}`) as Promise<
	...
	async fetchAllProductsOfCategories(id) {
		return this.get(`/api/product-categories?handle=${id}`) as Promise<PaginatedResponse<Category>>
	}

	async getMegamenu() {
		return this.get('/api/categories/megamenu') as Promise<PaginatedResponse<Category>>
	}
}
