...
export class CartService extends BaseService {
	...
	async addToCart({
		...
		if (body.qty === -9999999) {
			...
			if (lineId) {
				res = (await this.post(`/api/carts/${cartId}/line-items/${lineId}`, body)) as Cart
			} else {
	...
	async removeCart({ cartId, lineId = null }: { cartId: string; lineId: string | null }) {
	...
	async updateCart2({
		...
		if (shippingAddress) {
			...
			address_data = (await this.post('/api/address', body.shipping_address)) as Address
		...
		if (billingAddress && !isBillingAddressSameAsShipping) {
			...
			address_data = (await this.post('/api/address', body.billing_address)) as Address
