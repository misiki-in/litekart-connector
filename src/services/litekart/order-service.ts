...
export class OrderService extends BaseService {
	...
	async listOrdersByParent({ orderNo, cartId }: { orderNo: string | null; cartId: string | null }) {
		return this.get(`/api/orders/list-by-parent?order_no=${orderNo}&cart_id=${cartId}`) as Promise<
			PaginatedResponse<Order>
		>
	}
	...
	async fetchTrackOrder(id: string) {
		return this.get(`/api/orders/list-by-parent?id=${id}`) as Promise<PaginatedResponse<Order>>
	}
	...
	async cashfreeCheckout({ address, paymentMethod, prescription, origin }: any) {
		return this.get('/api/orders/me') as Promise<Order>
	}

	async razorpayCheckout({ address, paymentMethod, prescription, origin }: any) {
		return this.get('/api/orders/me') as Promise<Order>
	}
	...
	async listPublic() {
		return this.get('/api/orders/public/list') as Promise<PaginatedResponse<Order>>
	}

	async getOrderByEmailAndOTP({ email, otp }: { email: string; otp: string }) {
		return this.get(`/api/orders-public/list?otp=${otp}&email=${email}&sort=-createdAt`) as Promise<
			PaginatedResponse<Order>
		>
	}

	async buyAgain() {
		return this.get('/api/orders/buy-again') as Promise<PaginatedResponse<Order>>
	}
}
